...
//  peafowl- reset command
static void process_reset_command(conn *c, token_t *tokens, const size_t ntokens) {
    peafowl_reset_sched_stats();
    out_string(c, "OK");
}
...
static void process_command(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        // =peafowl start reset
    } else if (strcmp(tokens[COMMAND_TOKEN].value, "reset") == 0) {
        process_reset_command(c, tokens, ntokens);
