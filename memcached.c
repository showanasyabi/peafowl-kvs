...
static void settings_init(void) {
    ...
    settings.thread_affinity = false; // peafowl
    settings.thread_affinity_offset = 0; // peafowl
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    /**
     * Initialize peafowl members of connection data structures
     */
    c->rate = 0;
    ...
    c->home = -1;
    c-> is_guest = false;
    c->last_sampling_time = current_time;
    c->outlier_det_count = 0;
    c->outlier_det_M2 = 0;
    c->outlier_mean = 0;
    c->outlier_standard_dev = 0;
...
/**
 *  peafowl: a mechanism for enabling and disabling idle state
 */
static int get_number_of_c_states() {
    int file_count = 0;
    DIR * dirp;
    struct dirent * entry;
...
static
unsigned int cpuidle_state_write_file(unsigned int cpu, const char *fname, const char *value, size_t len) {
    char path[255];
    int fd;
    unsigned int i;
    ssize_t numwrite = 0;
    int cstates = get_number_of_c_states();
    for (i = 0; i < cstates; i++) {
        snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%u/cpuidle/state%u/%s",cpu, i, fname);
        if(settings.verbose > 0) {
            fprintf(stderr, "Info: opening path %s on cpu %u\n", path, cpu);
        }
        fd = open(path, O_WRONLY);
        if (fd == -1) {
            fprintf(stderr, "Error: failed to open idle state file %u on cpu %u (run it in root mode)\n", i, cpu);
            return 0;
        }
        numwrite = write(fd, value, len);
        if (numwrite < 1) {
            fprintf(stderr, "Error: failed to disable idle state %u on cpu %u\n", i, cpu);
            close(fd);
            return 0;
        }
        close(fd);
 ...
 */
int cpuidle_states_disable(unsigned int cpu, unsigned int disable) {
    char value[255];
    int bytes_written;
    snprintf(value, 255, "%u", disable);
    bytes_written = cpuidle_state_write_file(cpu, "disable", value, sizeof(disable));
    if (bytes_written) {
        if (settings.verbose > 0) {
            fprintf(stderr, "Successfully triggered the idle states on cpu %u\n", cpu);
        }
        return 0;
    }
    return -3;
...
/**
 * Peafowl outlier detection: checks if the new rate for a connection is
 * outlier- We use z-score approach for outlier detection
 */
static bool outlier_detection(conn *c, double new_rate) {
    double z = 0;
    if (c->outlier_mean != 0  && c->outlier_standard_dev != 0) {
        z = (new_rate - c->outlier_mean) / c->outlier_standard_dev;
    }

    c->outlier_det_count += 1;
    double  delta = new_rate - c->outlier_mean;
    c->outlier_mean += delta / c->outlier_det_count;
    double delta_2 = new_rate -  c->outlier_mean;
    c->outlier_det_M2 += delta * delta_2;
    c->outlier_standard_dev = sqrt(c->outlier_det_M2 / c->outlier_det_count);
    return  (z >= 3 || z <= -3);
}
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_new_cmd:
            ...
            /**
             * quantify the load or rate of the connection
             */
            ...
            if (current_time - c->last_sampling_time >= peafowl.monitoring_period) {
                ...
                double new_rate = (double) c->num_ops_over_last_window / (current_time - c->last_sampling_time);
                if (!outlier_detection(c , new_rate)) {
                    c->rate = new_rate;
                }
            ...
            /* sends a signal to peafowl scheduler if it is not transferring connections now*/
            if(current_time - c->thread->last_signaling_time >= peafowl.monitoring_period && !peafowl.transferring_epoch) {
                c->thread->last_signaling_time = current_time;
                // If the worker is already ignored  by scheduler, shrink the ignorance time
                if( c->thread->ignored_time_window > 0) {
                    c->thread->ignored_time_window --;
                }
        ...
        case conn_closing:
            ...
            stop = true;
            /**
             * Peafowl : here when a connection is closed, we update the load of its worker
             */
            c->thread->current_load -= c->rate;
            ...
            if (!c->is_guest) {
                c->thread->resident_load -= c->rate;
                if (c->thread->resident_load < 0) {
                    c->thread->resident_load = 0;
                }
            } else {
    ...
    /* here a worker checks if it is instructed to gives up  a connection */
    if(c->thread != NULL && peafowl.scale_down_worker == c->thread->index && peafowl.transferring_epoch && c->thread->instructed_send_conn) {
        c->thread->instructed_send_conn = false;
        ...
        if(c->thread->num_active_conn < 1) {
            ...
            c->thread->idle_state_enabled = true;
            cpuidle_states_disable(c->thread->index + 1, 0);
        }
        // transfer the  connection
        transfer_connection(c);
    }



    /*
     * Scaleup process. A worker sends gest connection home,
     * if its current load approaches peak load
     * 1) first check for significant increase in the load of  the current connection
     * 2) if current load is greater than 90% of the peak load, send guest connections home
     */

    if (c->thread != NULL && c->is_guest) {
        int window_length = (current_time - c->last_sampling_time > 0) ? current_time - c->last_sampling_time : 1;
        if (c->num_ops_over_last_window >= 1.1 * peafowl.monitoring_period * c->rate) {
            double new_conn_rate = c->num_ops_over_last_window / window_length;
            c->thread->current_load -= c->rate;
            c->rate = new_conn_rate;
            c->thread->current_load += c->rate;
            c->thread->capacity = c->thread->peak_load - c->thread->current_load;
        }

        if (c->thread->current_load > 0.9 * c->thread->peak_load) {
            c->thread->ignored_time_window = 30; // for 30  seconds ignore this thread for scheduling
            c->thread->num_active_conn--;
            c->thread->current_load -= c->rate;
...
static void usage(void) {
    ...
    printf("-Q                        peafowl: set distinct cpu affinity for threads, round-robin\n");
    printf("-O                        peafowl: set cpu affinity offset, starts from this core (default: 0)\n");
...
int main (int argc, char **argv) {
    ...
    char *shortopts =
          ...
          "P:"  /* save PID in file */
          "Q"   /* peafowl: Thread Affinity */
          "O:"  /* peafowl: Affinity offset */
          "f:"  /* factor? */
    ...
    const struct option longopts[] = {
        ...
        {"affinity-offset", required_argument, 0, 'O'}, // peafowl
    ...
    while (-1 != (c = getopt(argc, argv, shortopts))) {
        ...
        case 'O':
            ...
            if (settings.thread_affinity_offset < 0) {
                fprintf(stderr, "Peafowl: Core offset must be greater than 0 and less than number of cpus \n");
                return 1;
            }
            break;
            //  peafolw
    ...
    /* peafowl: pin the main thread to core 0 - */
