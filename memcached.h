...
struct settings {
    ...
    bool thread_affinity;      /*  Peafowl: we use this variable to pin threads/workers to distinct CPU cores  */
    int thread_affinity_offset;  /*  Peafowl: we use this variable to pin threads/workers to distinct CPU cores  */
...
typedef struct {
     ...
     *  Peafowl attributes
     */
    double current_load;        /* the current load of the thread/worker */
    double resident_load;       /* the current load of worker accounting only the resident connections */
    double peak_load;           /* the maximum load that is  hosted in the worker so far */
    double capacity;            /* capacity = peak_load - current_load */
    long num_active_conn;       /* the number of active connections currently hosted in the worker */
    int index;                  /* peafowl id for the worker thread */
    rel_time_t last_signaling_time; /* each worker signal scheduler every once a while */
    bool active;                /* true if the worker hosts at least one connection; otherwise; it is false */
    bool idle_state_enabled;    /* true  if idle state is enabled in the system */
    int ignored_time_window;     /* if a worker sends guest connections home. Scheduler ignores this worker and the destination worker for a time window (30s) */
    struct event peafowl_sched_event;  /* peafowl scheduler events */
    bool instructed_send_conn;     /* if true, the scheduler instructs this worker to gives up a connection*/
    int send_peafowl_msg_fd;
    int reciv_peafowl_msg_fd;
...
//  here we declare main_base for the use thread.c. It is defined in memcached.c
extern struct event_base *main_base;

struct peafowl_settings {
    unsigned int monitoring_period;         /* determines the rate of sampling */
    int num_active_workers;                 /* the number of workers that host at least one connection */
    int scale_down_worker;                  /* the worker form which connections migrate */
...
struct conn {
    ...
    /**
     * Peafowl members of connection data structure
     */
    double rate;                           /* the rate at which the connection sending KV requests*/
    long num_ops_over_last_window;          /*  number of operations over a window of time */
    bool on_load;                          /* shows if the connection load contributes to thread load */
    rel_time_t last_sampling_time;
    int home;                                /* home is the original   worker that hosts conn  for the first time*/
    bool is_guest;                           /* shows if the connection is currently not home and is a guest in other workers*/
    long outlier_det_count;          /* online avg and standard deviation using Welford algorithm (used for peafowl outlier detection) */
    double outlier_det_M2;             /* online avg and standard deviation using Welford algorithm (used for peafowl outlier detection) */
    double outlier_mean;
    double outlier_standard_dev;
...
void transfer_connection(conn *c); // peafowl
void  send_conn_home(conn *c);     // peafowl
...
void peafowl_reset_sched_stats(void); // peafowl
int cpuidle_states_disable(unsigned int cpu,
                           unsigned int disable); // peafowl
#ifdef EXTSTORE
